...
export const getPatientByMatric = async (req, res, next) => {
  try {
    // Extract patient matriculation number from URL parameters
    const { matric } = req.params;

    // Find the patient by matriculation number
    const patient = await Patient.findOne({ matricNumber: matric });

    if (!patient) {
      console.log("No patient found with matricNumber:", matric);
      return errorResMsg(res, 404, {
        message: "Patient not found",
      });
    }
